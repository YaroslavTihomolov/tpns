...
def check(perceptron: Perceptron, data_set, original_data_set):
    ...
    false_negative = 0
    false_positive = 0
    max_mushroom_value = original_data_set['Price'].max()
    min_mushroom_value = original_data_set['Price'].min()
    predictions = []
    actuals = []
    max_val = 0
    mse = 0
    ...
    for i in range(len(original_data_set.values)):
        ...
        val = perceptron.run()
        perceptron.clean()
        actual_value = un_normalize(val, max_mushroom_value, min_mushroom_value)
        expected_value = original_data_set.values[i][-1]
        predictions.append(actual_value)
        actuals.append(expected_value)
        mse += (expected_value - actual_value) ** 2
        mae += abs(actual_value - expected_value)
        # print([actual_value, expected_value])
        max_val = max(max_val, abs(actual_value - expected_value))
    ...
    # print(max_val)
    print("step")
    mse /= len(original_data_set.values)
    print('mse : ' + str(mse))
    rmse = mse ** 0.5
    print('rmse : ' + str(rmse))
    mae /= len(original_data_set.values)
    print('mae : ' + str(mae))
    print('\n')
...
def educate_and_check(data_set, original_data_set, section_count):
    ...
    for i in range(section_count):
        perceptron = Perceptron(create_net([len(data_set.columns) - 1, 4, 3]))
        data = []
...
if __name__ == "__main__":
    ...
    perceptron = Perceptron(create_net([len(laptops.columns) - 1, 4, 2]))
    # run(perceptron, mushrooms, 1, mushrooms1)
    # educate_and_check(mushrooms, mushrooms1, 4)
    educate_and_check(laptops, laptops1, 4)
